...
const ShaderMaterial = ({ source, uniforms, maxFps = 60 }: ShaderMaterialProps) => {
  ...
  const getUniforms = () => {
      ...
      switch (uniform.type) {
        ...
        case "uniform3f":
          preparedUniforms[uniformName] = {
            value: new THREE.Vector3().fromArray(uniform.value as number[]) as VectorLike,
            type: "3f",
          };
          break;
        case "uniform1fv":
        ...
        case "uniform3fv":
          preparedUniforms[uniformName] = {
            value: (uniform.value as number[][]).map(
              (v) => new THREE.Vector3().fromArray(v) as VectorLike,
            ),
            type: "3fv",
          };
          break;
        case "uniform2f":
          preparedUniforms[uniformName] = {
            value: new THREE.Vector2().fromArray(uniform.value as number[]) as VectorLike,
            type: "2f",
          };
          break;
        default:
    ...
    preparedUniforms.u_resolution = {
      value: new THREE.Vector2(size.width * 2, size.height * 2) as VectorLike,
